  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      <View style={styles.summaryRow}>
        <SummaryCard label="Lotes totales" value={weeklyBatches} colors={colors} typography={typography} />
        <SummaryCard label="Kg malta" value={weeklyMalt.toFixed(0)} colors={colors} typography={typography} />
        <SummaryCard label="Estilos" value={[...new Set(plans?.map((p) => p.style) ?? [])].length} colors={colors} typography={typography} />
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <PlanCard plan={item} onDelete={handleDelete} onGenerateOrders={() => setPreviewPlan(item)} />
          )}
          ListEmptyComponent={
            <EmptyState
              icon="🗓️"
              title="Sin plan de producción"
              subtitle="Agrega lotes para ver el plan semanal"
            />
          }
        />
      )}

      <Pressable style={[styles.fab, { backgroundColor: colors.gold, shadowColor: colors.gold }]} onPress={() => setShowForm(true)}>
...
function PlanCard({
  plan, onDelete, onGenerateOrders,
}: {
  plan: ProductionPlan;
  ...
  return (
    ...
    >
      ...
      <View style={planCardStyles.right}>
        <View
          style={[
            planCardStyles.daysBadge,
            { backgroundColor: colors.card, borderColor: colors.border },
            isToday && { borderColor: colors.gold, backgroundColor: colors.goldDim + "33" },
          ]}
        >
          <Text style={[typography.label, { fontSize: 9, color: isToday ? colors.gold : colors.textMuted }]}>
            {days}
          </Text>
        </View>
        <View style={planCardStyles.actions}>
          {isOrdered ? (
            ...
            <Pressable
              onPress={onGenerateOrders}
              hitSlop={8}
              style={[planCardStyles.orderBtn, { borderColor: colors.goldDim }]}
            >
                    ...
                    {ordersNeeded.map((item) => (
                      <View key={item.materialId} style={[styles.previewRow, { borderBottomColor: colors.border }]}>
                        ...
                        <View style={styles.previewRight}>
                          <Text style={[typography.bodySmall, { fontWeight: "700", color: colors.gold }]}>
                ...
                {STYLES.map((s) => (
                  <Pressable
                    ...
                    style={[
                      ...
                      s === selectedStyle && { borderColor: colors.gold },
                    ]}
